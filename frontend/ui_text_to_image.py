...
def ui_text_to_image(
    ...
    with gr.TabItem("Text-to-Image", id='txt2img_tab'):
        ...
        with gr.Row(elem_id='body').style(equal_height=False):
            ...
            # Start Column
            with gr.Column(elem_id='txt2img_start_column'):
                txt2img_ui['width'] = gr.Slider(
                    minimum=64, maximum=1024, step=64, label="Width", value=txt2img_defaults["width"]
                )
                txt2img_ui['height'] = gr.Slider(
                    minimum=64, maximum=1024, step=64, label="Height", value=txt2img_defaults["height"]
                )
                txt2img_ui['cfg'] = gr.Slider(
                    minimum=-40.0, maximum=30.0, step=0.5,
                ...
                txt2img_ui['seed'] = gr.Textbox(
                    label="Seed (blank to randomize)", lines=1, max_lines=1, value=txt2img_defaults["seed"]
                )
                txt2img_ui['batch_size'] = gr.Slider(
                    minimum=1, maximum=50, step=1, label='Images per batch', value=txt2img_defaults['batch_size']
                )
                txt2img_ui['batch_count'] = gr.Slider(
                    minimum=1, maximum=50, step=1, label='Number of batches to generate', value=txt2img_defaults['n_iter']
                )
            ...
            # Output Column
            with gr.Column(elem_id="txt2img_output_column"):
                ...
                with gr.Box():
                    ...
                    with gr.Row(elem_id='txt2img_actions_row'):
                        txt2img_ui['to_clipboard_btn'] = gr.Button("Copy to clipboard").click(
                            ...
                            outputs=[],
                            _js=call_JS("copyImageFromGalleryToClipboard", fromId="txt2img_gallery_output")
                        )
                        txt2img_ui['to_img2img_btn'] = gr.Button("Push to img2img")
                        txt2img_ui['to_imglab_btn'] = gr.Button("Send to Lab")

                # Generation parameters
                with gr.Box():
                    txt2img_ui['output_params'] = gr.Highlightedtext(
                        label="Generation parameters", interactive=False, elem_id='output_parameters',
                    )
            ...
            # End Column
            with gr.Column(elem_id='txt2img_end_column'):
                ...
                with gr.Tabs():
                    with gr.TabItem('Simple'):
                        ...
                        txt2img_ui['submit_on_enter'].change(
                            lambda x: gr.update(max_lines=1 if x == 'Yes' else 25), txt2img_ui['submit_on_enter'],
                            txt2img_ui['prompt']
                        )

                    with gr.TabItem('Advanced'):
                        txt2img_ui['toggles'] = gr.CheckboxGroup(
                            label='', choices=txt2img_ui['toggles'], value=txt2img_toggle_defaults, type="index",
                        )
                        ...
                        txt2img_ddim_eta = gr.Slider(
                            minimum=0.0, maximum=1.0, step=0.01, label="DDIM ETA",
                            value=txt2img_defaults['ddim_eta'], visible=False,
                        )
                        txt2img_variant_amount = gr.Slider(
                            minimum=0.0, maximum=1.0, label='Variation Amount',
                            value=txt2img_defaults['variant_amount'],
                        )
                        txt2img_variant_seed = gr.Textbox(
                            label="Variant Seed (blank to randomize)", lines=1, max_lines=1,
        ...
        txt2img_ui['inputs'] = [
            txt2img_ui['prompt'], txt2img_steps, txt2img_sampling, txt2img_ui['toggles'],
            txt2img_realesrgan_model_name, txt2img_ddim_eta, txt2img_ui['batch_count'],
            txt2img_ui['batch_size'], txt2img_ui['cfg'], txt2img_ui['seed'], txt2img_ui['height'], txt2img_ui['width'],
            txt2img_embeddings, txt2img_variant_amount, txt2img_variant_seed,
        ]

        txt2img_ui['outputs'] = [
            txt2img_ui['gallery'], txt2img_ui['output_seed'], txt2img_ui['output_params'], output_txt2img_stats
        ]
        ...
        if txt2img_ui['job_ui']:
            txt2img_func, txt2img_ui['inputs'], txt2img_ui['outputs'] = txt2img_ui['job_ui'].wrap_func(
                func=txt2img_func,
                inputs=txt2img_ui['inputs'],
                outputs=txt2img_ui['outputs']
            )
            use_queue = False
        else:
            use_queue = True
        ...
        txt2img_ui['width'].change(
            fn=uifn.update_dimensions_info, inputs=[txt2img_ui['width'], txt2img_ui['height']],
